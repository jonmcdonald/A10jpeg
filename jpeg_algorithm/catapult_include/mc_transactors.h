...
protected:
  mc_rsc_block_base() { _is_disabled = false; }
...
struct mc_transactor_event_module : sc_module
{
  ...
  bool log_event(mc_transactor_event event_mask) {
...
protected:
  ...
  mc_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit)
    : mc_transactor_event_module(name)
    , _CH(BaseBit+WordWidth-1), _BaseBit(BaseBit), _WordWidth(WordWidth)
    , _RH(int(ceil(double(Telemwidth * Telemcnt) / _WordWidth)) + BaseAddr - 1), _BaseAddr(BaseAddr)
    , _empty(false)
...
public:
  ...
  mc_input_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, bool disable_on_empty=false)
    : mc_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
    , in_fifo("in_fifo")
    , _ld_count(0)
...
public:
  ...
  mc_output_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit)
    : mc_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
    , out_fifo("out_fifo")
    , _dmp_count(0)
  {}
...
{
  mc_inout_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit)
    : mc_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
    , mc_input_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
    , mc_output_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
  {}
};
...
public:
  ...
  mc_channel_input_transactor(const sc_module_name& name, int BaseAddr, int WordWidth, int BaseBit, bool disable_on_empty=false)
    : mc_transactor<Tclass,Telemwidth,Tsigned>(name,BaseAddr,WordWidth,BaseBit)
    , in_fifo("in_fifo")
    , _ld_count(0)
    , _disable_on_empty(disable_on_empty)
...
//#endif
  ...
  void start_wait_count(unsigned short iodir) {
    ...
    if ((_wait_ctrl[iodir].elements < mc_wait_ctrl::FIXED_INTERVAL) ||  ((_wait_elem_cntr[iodir] % i) == 0)) {
      ...
      _wait_cycles_changed[iodir].notify();
      _wait_random_interval[iodir] = (_wait_ctrl[iodir].interval > 0) ? scverify_rand(_wait_ctrl[iodir].interval) : 0;
    }
  ...
  void wait_controller(unsigned short iodir) {
    ...
    if (_wait_cycles_cntr[iodir] == 0) {
      ...
      drive_wait_signals(true,iodir);
      if (io_request(iodir) && (_wait_cycles_cntr[iodir] != -1)) _wait_cycles_cntr[iodir]--;
    }
    }
  }
