...
void JPEG_ENCODER_pv::cb_write_start(unsigned int newValue) {
  ...
  else 
  {
    ...
    if (num_rows > MAX_ROWS)
        num_rows = MAX_ROWS;
#else
    unsigned num_rows = inputlength & 0xFFFF;
#endif
...
#else
    unsigned num_cols = (inputlength >> 16) & 0xFFFF;
    cout <<name()<<" @ "<<sc_time_stamp()<<" master_read size= "<< 4 * num_rows * num_cols <<endl;
    master_read (inputaddr, data_temp, 4 * num_rows * num_cols);
#endif

#ifdef OLD_JPEG_REGS
    for (int i = 0; i < inputlength; i = i + 4) {
#else
    for (int i = 0; i<(4*num_rows*num_cols); i = i + 4) {
#endif
        rgb_t rgb_temp;
    ...
    // write jpeg image
    cout <<name()<<" @ "<<sc_time_stamp()<<" master_size size= "<< jpeg_data_len_temp <<endl;
