...
unsigned int bmp::getpixel(FILE * fp) {
   ...
   switch (infoheader.bitcount) {
      case 1: bitsleft--; ret = (unsigned)pixelbuffer & 128; pixelbuffer >>= 1; break;
      case 2: bitsleft -= 2; ret = (unsigned)pixelbuffer & 192; pixelbuffer >>= 2; break;
      case 4: bitsleft -= 4; ret = (unsigned)pixelbuffer & 240; pixelbuffer >>= 4; break;
      case 8: bitsleft = 0; ret = (unsigned)pixelbuffer; break;
      case 16: bitsleft = 0; ret = (unsigned)(pixelbuffer << 8) | (unsigned) getbyte(fp); break;
      case 24: bitsleft = 0; 
         ret = ((unsigned)pixelbuffer << 16) | 
            ((unsigned)getbyte(fp) << 8) |
            ((unsigned)getbyte(fp)); break;
      default: assert(1 == 0);
   }
   ...
   for (unsigned i = 0; i < infoheader.height; i++) {
      ...
      for (unsigned j = 0; j < infoheader.width; j++) {
         if (colortable) {
            ...
            index[2] = (unsigned char) (p & 255); p >>= 8;
            index[1] = (unsigned char) (p & 255); p >>= 8;
