...
public:    
  ...
  mgc_sysc_ver_array1D(const mgc_sysc_ver_array1D &rhs) : _lower(-1), _upper(rhs._upper), _p(rhs._p), _x(rhs._x)
  {
    if (_p) _p->inc();
  }
  ...
  ~mgc_sysc_ver_array1D() { if (_p && _p->dec() == 0) delete _p; }
  ...
  const Tclass& operator[](int i) const { 
    if (_p) return _p->val[i]; else return _x;
  }
