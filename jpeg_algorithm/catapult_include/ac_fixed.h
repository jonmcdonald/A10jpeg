 ...
 *  Release Build   : 3.4.0                                               *
 *                                                                        *
 *  Copyright 2005-2014, Calypto Design Systems, Inc.,                     *
 *                                                                        *
 *  All Rights Reserved.                                                  *
 *                                                                        *
 **************************************************************************
...
#endif
#if (defined(__GNUC__) && __GNUC__ >= 4 && __GNUC_MINOR__ >= 6 && !defined(__EDG__))
...
namespace ac {
  ...
  struct rt_ac_fixed_T< ac_fixed<W2,I2,S2> > {
    ...
    typedef ac_fixed<W2,I2,S2> equiv;
    template<int W, int I, bool S>
    struct op1 {
  ...
  struct rt_ac_fixed_T< ac_int<W2,S2> > {
    ...
    typedef ac_fixed<W2,W2,S2> equiv;
    template<int W, int I, bool S>
    struct op1 {
      typedef ac_fixed<W,I,S> fx_t;
...
// function to initialize (or uninitialize) arrays
  ...
  inline ac_fixed<54,1,true> frexp_d(double d, ac_int<11,true> &exp) {
    enum {Min_Exp = -1022, Max_Exp = 1023, Mant_W = 52, Denorm_Min_Exp = Min_Exp - Mant_W};
    int exp_i;
    double f0 = frexp(d, &exp_i); 
    AC_ASSERT(exp_i <= Max_Exp, "Exponent greater than standard double-precision float exponent max (+1023). It is probably an extended double");
    AC_ASSERT(exp_i >= Denorm_Min_Exp, "Exponent less than standard double-precision float exponent min (-1022). It is probably an extended double");
    int rshift = exp_i < Min_Exp ? Min_Exp - exp_i : (exp_i > Min_Exp && f0 < 0 && f0 >= -0.5) ? -1 : 0; 
    exp = exp_i + rshift;
    ac_int<Mant_W+2,true> f_i = f0 * ((Ulong) 1 << (Mant_W + 1 -rshift));
    ac_fixed<Mant_W+2,1,true> r;
    r.set_slc(0, f_i);
    return r; 
  }
  inline ac_fixed<25,1,true> frexp_f(float f, ac_int<8,true> &exp) {
    enum {Min_Exp = -126, Max_Exp = 127, Mant_W = 23, Denorm_Min_Exp = Min_Exp - Mant_W};
    int exp_i;
    float f0 = frexpf(f, &exp_i); 
    AC_ASSERT(exp_i <= Max_Exp, "Exponent greater than standard single-precision float exponent max (+127). It is probably an extended float");
    AC_ASSERT(exp_i >= Denorm_Min_Exp, "Exponent less than standard single-precision float exponent min (-126). It is probably an extended float");
    int rshift = exp_i < Min_Exp ? Min_Exp - exp_i : (exp_i >= Min_Exp && f0 < 0 && f0 >= -0.5) ? -1 : 0; 
    exp = exp_i + rshift;
  ...
  inline ac_fixed<53,0,false> frexp_sm_d(double d, ac_int<11,true> &exp, bool &sign) {
    enum {Min_Exp = -1022, Max_Exp = 1023, Mant_W = 52, Denorm_Min_Exp = Min_Exp - Mant_W};
    int exp_i;
    bool s = d < 0;
    ...
    ac_fixed<Mant_W+1,0,false> r;
    r.set_slc(0, f_i);
    sign = s;
    return r;
  }
  inline ac_fixed<24,0,false> frexp_sm_f(float f, ac_int<8,true> &exp, bool &sign) {
    enum {Min_Exp = -126, Max_Exp = 127, Mant_W = 23, Denorm_Min_Exp = Min_Exp - Mant_W};
    int exp_i;
    bool s = f < 0;
