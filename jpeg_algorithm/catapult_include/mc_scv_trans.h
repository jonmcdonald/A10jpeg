...
namespace CCS_SCV {
  ...
  SC_MODULE(TXN_MOD_fifo_in) {
    public:
      ...
      // fifo_size feeds LV to the 'sizez' port in RTL MODEL (mgc_chan_in only)
    ...
    public:
      ...
      void load_data_proc() {
        ...
        // Extracting data for first read by RTL
        data_T d = fifo_data.read();
        ...
        while(true) {
          ...
          if (fifo_data.num_available() && (vl == sc_dt::Log_1)) {
    ...
    public:
      ...
      void load_data_proc() {
        ...
        while (true) {
          wait();
          const sc_dt::sc_logic vl = valid.read();
          if (!!fifo_data.num_free() && (vl == sc_dt::Log_1)) {
            d = data.read();
